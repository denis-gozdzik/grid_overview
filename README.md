...
The next deterministic layer adds `Profile_Usefulness`, `Profile_Discovery`, `Profiles` and `Profile_Objects`. Network core fingerprint v1 uses effective lease time, DNS servers, domain name and a derived gateway convention. Range core fingerprint v1 adds the Range association family before the same four functional fields. Gateway values are normalized structurally (for example `FIRST_USABLE`, `LAST_USABLE`, or `HOST_OFFSET_3`) instead of hashing literal addresses. Stable `NET-FP1-...` / `RNG-FP1-...` IDs are SHA-256-derived from canonical payloads and do not depend on prevalence rank. Feature overlays remain descriptive and do not split the core fingerprint. Objects with unresolved required core evidence are kept outside normal profiles; Range `NONE` stays explicitly not applicable. See [Profile fingerprint v1](docs/profile-fingerprint-v1.md).
