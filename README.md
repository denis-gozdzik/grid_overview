...
`page_size` must be an integer from 1 to 1000; booleans, fractions, strings and out-of-range values fail configuration validation. Every RAW/effective query, including per-parent child queries, starts with the configured `_max_results`. Subsequent RAW requests use only the WAPI `_page_id`. For effective inheritance queries, NIOS 9.x lab evidence showed that `_page_id` alone can drop scalar inheritance wrappers from page 2 onward, so continuation GETs reassert `_inheritance=True` together with `_page_id`. This preserves the effective response shape while remaining read-only. There is no automatic page-size adjustment or concurrent collection.
...
Reports include `current_state_inventory.xlsx`, `standardization_decisions.template.yaml`, `current_state_summary.md`, and `manual_review.md`. The workbook retains filters, freeze panes, readable widths, object inventories, and coverage, with separate DHCP raw, effective scalar, and option information. Source refs are retained alongside human-readable source levels and objects. Nested values are serialized for Excel, and values resembling formulas are written as literal text.

Start with `Overview`. It is a workshop-oriented dashboard rather than an API dump: environment KPIs, assessment coverage, collection metadata, standardization counts and the highest-priority observed hotspots appear first. `Standardization`, `Decisions`, `Exceptions` and `Grid_Comparison` then turn the same normalized evidence into a decision workflow. Common observed values are descriptive only; only a human `APPROVED` decision defines a target. Standardization rows and decision IDs are scope-specific (`Grid`, `Member`, `Network`, `Range`), so percentages never mix hierarchy levels and each percentage carries an explicit object-population denominator. A mismatch is never labeled a deviation before that target exists. Detailed source refs and RAW values remain in `DHCP_Effective`, `DHCP_Options` and `DHCP_Raw`.
...
`Profile_Readiness`, immediately after `Overview`, assesses evidence for 21 Network and 18 Range candidate profile inputs. Network readiness uses an evidence-backed DHCP-relevant Network candidate population rather than every IPAM Network. Range readiness segments RAW `server_association_type` evidence into MEMBER, FAILOVER, MS_SERVER, NONE and unknown/other cases, then applies parameter-specific populations: DHCP options use associated ranges, while Infoblox scalar inputs use Infoblox-managed ranges plus any MS_SERVER objects that actually expose authoritative scalar evidence. `MS_SERVER` is never translated into `NOT_CONFIGURED`. `Profile_Populations` shows the descriptive denominators and excluded segments.

Functional profile proof compares effective values independently of source provenance: identical COMPLETE non-multisource values from different source levels count once as confirmed, while the original source/inheritance evidence remains preserved for future source-profile analysis. Recognized authoritative effective option groups can prove bounded assessed option absence; malformed/unwrapped responses remain unresolved. Composite inputs such as PXE lease enablement remain deferred. A compact Overview summary links to the matrix. `READY` describes evidence availability, not usefulness, compliance or an approved standard. See [Profile Input Readiness](docs/profile-discovery-v1.md) for candidate IDs, thresholds, denominators and inheritance diagnostics.

Deterministic discovery is reported separately in `Profile_Usefulness`, `Profile_Fingerprints`, `Profile_Assignments` and `Profile_KPIs`. Version 1 exact fingerprints use effective lease time, DNS servers, domain name and a topology-relative gateway convention; Range fingerprints additionally include the server-association family. Literal gateway addresses are converted to conventions such as `FIRST_USABLE`, `LAST_USABLE` and `HOST_OFFSET_N`. Objects with unresolved required core inputs stay outside normal fingerprints. Fingerprint IDs are canonical-payload SHA-256 identifiers and do not depend on prevalence rank. Remaining candidate inputs are descriptive feature overlays; no profile is automatically selected as a standard.

Context stays outside fingerprint identity. `Profile_Context` summarizes bounded Network View, association, DHCP-relevance, EA presence/value, comment and core source/inheritance observations for each resolved fingerprint. `Profile_Relationships` and `Profile_Relationship_Assignments` describe parent Network↔Range profile pairs and preserve unresolved/not-applicable relationships explicitly. Recurring context or relationship patterns remain observations, not semantic names or approved standards. See [Profile fingerprint v1](docs/profile-fingerprint-v1.md).
...
The topology increment adds reusable DHCP objects only. Fixed-address objects, filters, DNS, DDNS and Extensible Attributes are not extended by it. Cross-Grid analysis now aggregates the standardization questions already supported by normalized DHCP/PXE/DDNS evidence and explicitly reports when only one Grid is present. It remains descriptive: it does not establish object correspondence across differing site structures, approve a target, or exhaustively compare filters, templates and EAs. Further collector scope requires runtime schema/response evidence.
